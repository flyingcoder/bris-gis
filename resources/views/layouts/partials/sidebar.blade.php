...
<!--             <li class="{!! Request::is('homeUI') ? 'active' : '' !!}" ><a href="{{ route('home.indexUI') }}"><i class='fa fa-dashboard'></i> <span>Dashboard</span></a></li> -->
            ...
            @else
            <li class="{!! Request::is('households') ? 'active' : '' !!}"><a onclick="document.body.style.cursor='wait'; return true;" href="{{ route('households.get', Auth::user()->with('barangayAdmin')->find(Auth::user()->id)->barangayAdmin->barangay_id )}}"><i class='fa fa-home '></i> <span>Households</span></a></li>
